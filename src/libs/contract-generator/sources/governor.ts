...
function setVotingParameters(
  ...
  c.setFunctionBody(
    [`return ${delayBlocks}; // ${opts.delay}`],
    functions.votingDelay
  );

  const periodBlocks = getVotingPeriod(opts);
  c.setFunctionBody(
    [`return ${periodBlocks}; // ${opts.period}`],
...
function addQuorum(c: ContractBuilder, opts: Required<GovernorOptions>) {
  if (opts.quorumMode === 'percent') {
    if (opts.votes !== 'erc20votes' && opts.votes !== 'erc721votes') {
      throw new OptionsError({
        quorumPercent:
          'Percent-based quorum is only available for ERC20Votes or ERC721Votes',
      });
    }
    ...
    let { quorumFractionNumerator, quorumFractionDenominator } =
